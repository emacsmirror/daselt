...
;;;; Constants
(defconst d-emacs-base-definition-types-list
  '(defun defmacro defconst defcustom defun* defalias defgroup define-derived-mode defvar-keymap)
  "List of definition macros for which `d-emacs-base-beginning-of-docstring' works.")

(defconst d-emacs-base-escape-kbd-regexps-list
...
(defconst d-emacs-base-unusual-docstring-positions-alist
                  '((defvar-keymap . 2))
...
;;;;; Files
(defun d-emacs-base-containing-directory-base-name (filepath)
                                     ...
                                     (save-excursion
                                       ...
                                       (let (rlist)
                                         (while (search-forward (symbol-name deftype) nil t)
                                           (save-excursion
                                             (beginning-of-defun)
                                             (push (d-emacs-base-definition-name) rlist)))
                                         rlist))))
                             d-emacs-base-definition-types-list))
      (set-buffer buf))))

;;;;; Arrays
(defun d-emacs-base-filter-obarray (predicate)
...
;;;;; Region operations
(defun d-emacs-base-read-region (&optional properties)
...
(defun d-emacs-base-replace-region (arg)
...
;;;;; Lists
(defun d-emacs-base-remove-indices (indlst)
...
(defun d-emacs-base-filter-list (lst pred)
...
(defun d-emacs-base-preimage (lst fun obj &optional keepobj eqpred)
...
(defun d-emacs-base-image (lst fun &optional eqpred)
...
(defun d-emacs-base-fiber-by-property (lst propfun &optional keepprops eqpred)
...
effects."
  ...
  (let* ((eqpred (or eqpred #'equal))
         (props (d-emacs-base-image lst propfun eqpred))
         (fibration (mapcar (lambda (prop)
                              (let ((fiber (d-emacs-base-preimage
                                            lst propfun prop keepprops eqpred)))
                                fiber))
                            props)))
    fibration))

(defun d-emacs-base-global-sections (llist)
  ...
  (mapcan (lambda (elt)
            (if (cdr llist)
                (mapcar (lambda (section)
                        ...
                        (d-emacs-base-global-sections (cdr llist)))
              (list (list elt))))
          (car llist)))

(defun d-emacs-base-recursive-sections (obj)
...
element that is a list. Then apply `d-emacs-base-global-sections' around it."
  (declare (pure t)
  ...
  (if (proper-list-p obj)
      (let ((formattedlist (mapcar (lambda (elt)
                                     (if (and elt (proper-list-p elt))
                                       ...
                                       (list elt)))
                                   obj)))
        (d-emacs-base-global-sections formattedlist))
    (list (list obj))))

(defun d-emacs-base-lisp-file-code (filename)
...
(defun d-emacs-base-make-list-if-not (obj)
...
(defun d-emacs-base-cl-mapcar-or-only (fun &rest objs)
...
one-element lists before the application."
  ...
  (apply #'cl-mapcar fun
         (mapcar (lambda (obj)
                   (d-emacs-base-make-list-if-not obj))
                 objs)))

(defun d-emacs-base-cons-to-list (cns)
...
(defun d-emacs-base-list-to-cons (lst)
...
(defun d-emacs-base-flatten-until (lst cnd)
...
(defun d-emacs-base-flatten-n-times (lst &optional n)
...
(defun d-emacs-base-reverse-alist-get (key alist &optional default testfn)
...
(defun d-emacs-base-sexp-end-position (&optional beg)
...
(defun d-emacs-base-remove-text-properties-from-string (str)
...
(defun d-emacs-base-string-exists-and-nonempty (str)
...
(defun d-emacs-base-concat-with-separators (separator &rest strs)
...
(defun d-emacs-base-intern-from-parts (&rest parts)
...
Make the result a symbol."
  (declare (ftype (function (&rest string) string))
           (side-effect-free t))
  (intern (apply #'d-emacs-base-concat-with-separators "-" parts)))

;;;;; Filling
(defun d-emacs-base-beginning-of-docstring ()
  "Move to the start of the docstring if point is within a definition.

Works for definitions in `d-emacs-base-definition-types-list'."
  (declare (ftype (function () void))
           (side-effect-free nil))
  (beginning-of-defun)
  (forward-char)
  (let* ((first-symbol (progn (mark-sexp)
                              (prog1 (d-emacs-base-read-region)
                                (deactivate-mark))))
         (types d-emacs-base-definition-types-list))
    (when (member first-symbol types)
      (let ((place (alist-get first-symbol d-emacs-base-unusual-docstring-positions-alist 3 #'eq)))
        (forward-sexp place)
        (search-forward "\"")
        (backward-char)))))

(defun d-emacs-base-fill-string-at-point-like-docstring ()
  ...
  (cl-flet ((fill-rest (beg &optional end)
              ...
              (let ((fillbeg (if (< (line-beginning-position) beg)
                ...
                (fill-region fillbeg (or end (progn (goto-char beg)
                                                    ...
                                                    (point)))))))
    (let ((beg (point))
          (end (d-emacs-base-sexp-end-position)))
...
(defun d-emacs-base-generate-newlines (k)
...
(defun d-emacs-base-surround-by-newlines (k l str)
  "Prepend STR with K newlines and append it with L newlines."
  (declare (ftype (function (integer integer string) string))
           (pure t))
  (concat (d-emacs-base-generate-newlines k) str (d-emacs-base-generate-newlines l)))

(defun d-emacs-base-prepend-newlines (k str)
  "Prepend K newlines before STR."
  (declare (ftype (function (integer string) string))
           (pure t))
  (d-emacs-base-surround-by-newlines k 0 str))

(defun d-emacs-base-append-newlines (k str)
  "Append K newlines before STR."
  (declare (ftype (function (integer string) string))
           (pure t))
  (d-emacs-base-surround-by-newlines 0 k str))

;;;;; Logical and set-theoretic operations
(defun d-emacs-base-forall-p (list predicate)
...
(defun d-emacs-base-complement (list1 list2 &optional compfun)
...
(defun d-emacs-base-powerlist (list &optional elt)
...
is used for recursion and should normally not be set by the user."
  ...
  (let ((powerlist (list nil))
    ...
    (if elt (mapcar (lambda (sublist) (append (list elt) sublist))
                    (d-emacs-base-powerlist (cl-loop for index from 0 to (cl-position elt list)
                                                     do (setq cutlist (remq (nth index list) cutlist))
                                                     finally return cutlist)))
      (cl-loop for elt in list
               do (setq powerlist (append powerlist (d-emacs-base-powerlist list elt)))
               finally return powerlist))))

(defun d-emacs-base-setequal (list1 list2 &optional elttest)
...
(defun d-emacs-base-remove-list-index (lst idx)
...
;;;;; Comparison
(defun d-emacs-base-leq-p (seq1 seq2)
...
(defun d-emacs-base-geq-p (seq1 seq2)
...
(defun d-emacs-base-compare-if-decidable (test arg1 arg2)
...
(defun d-emacs-base-compare-by-sequential-predicates (arg1 arg2 &rest predicates)
  "Compare ARG1 and ARG2 using the sequence of PREDICATES.

`d-emacs-base-compare-if-decidable' is called with ARG1, ARG2 and one predicate
after the other. If it returns `(t)', or `(nil)', this is returned by
`d-emacs-base-compare-by-sequential-predicates', otherwise the next predicate is
used.
...
effects."
  ...
  (cl-loop for pred in predicates
           do (let ((compval (d-emacs-base-compare-if-decidable pred arg1 arg2)))
                (if compval (cl-return compval)))))

;;;;; Insertion
(defun d-emacs-base-capture-inserted-text (fn &rest args)
...
(defun d-emacs-base-froundout (num)
...
(defun d-emacs-base-namecore (sym &optional pfx sfx)
...
If SYM does not have a PFX or SFX, ignore them."
  ...
  (let* ((pfx (or pfx ""))
         ...
         (match (progn (string-match (concat pfx "\\(.*\\)" sfx) name)
                       (match-string 1 name))))
    (if (d-emacs-base-string-exists-and-nonempty match)
        match
      name)))

;;;;; Recursion
(defun d-emacs-base-funcalls-recursively (obj funtests &optional recursetest formatfun eltcolfun lstcolfun restargs restargfun contt debug)
...
`d-emacs-base-funcalls-recursively' is applied to ELT with the same original
...
(defun d-emacs-base-funcall-recursively (obj fun test  &optional recursetest formatfun eltcolfun lstcolfun restargs restargfun contt debug)
...
See there for further explanation."
  (declare (ftype (function (t ; OBJECT
                             ...
                             boolean) ; DEBUG
                            t)))
  (d-emacs-base-funcalls-recursively obj `((,fun . ,test)) recursetest formatfun eltcolfun lstcolfun restargs restargfun contt debug))

;;;;; Conses
(defun d-emacs-base-recursively-act-on-proper-conses (list fun &optional lstcolfun)
...
The function uses `d-emacs-base-funcall-recursively' to handle traversal:
...
flat list."
  ...
  (let ((lstcolfun (or lstcolfun #'append)))
    (d-emacs-base-funcall-recursively list
                                      fun
                                      (lambda (idx lst)
                                        (let ((elt (nth idx lst)))
...
(defun d-emacs-base-recursive-get-cons (obj allist &optional testfn reverse)
...
effects."
  (declare (ftype (function (t list &optional (function (t t) boolean) boolean)
                            ...
                            list))
           (pure t))
  (d-emacs-base-recursively-act-on-proper-conses
   allist
...
;;;;; Drawing
(defun d-emacs-base-with-max-buffer-maybe-return (bufname fun)
...
;;;;; Macros
;;;;;; Function-generation
(defmacro d-emacs-base-def-by-forms (templates &rest mappings)
...
obj1 → b, obj → e obj1 → b, obj → f."
  (let* ((substitutions (mapcar #'car mappings))
         (substitution-cardinal (d-emacs-base-cardinal (length substitutions)))
                      ...
                      (mapcan
                       (lambda (defun-num)
                         (let* ((nth-values (mapcar
                                ...
                                (extended-values (d-emacs-base-recursive-sections nth-values))
                                (contexts (mapcar
                           ...
                           (mapcar (lambda (context)
                                   ...
                                   contexts)))
                       (d-emacs-base-cardinal lengths-set))))
                template-list))
            templates)
           :test #'equal)))
    (append '(progn) defun-list)))

(defmacro d-emacs-base-def-by-forms-by-variables (templates &rest mappings)
  "Like `d-emacs-base-def-by-forms' but each cdr of mappings should be a
variable name or otherwise evaluate to a list.

See `d-emacs-base-def-by-forms' for more documentation."
  (let ((newmappings (mapcar (lambda (mapping)
                               (cons (car mapping) (eval (cdr mapping))))
                             mappings)))
    `(d-emacs-base-def-by-forms ,templates ,@newmappings)))

;;;;; Navigation
(defun d-emacs-base-goto-min ()
  "Go to `point-min'."
  (declare (ftype (function () integer)))
  (goto-char (point-min)))

(defun d-emacs-base-goto-max ()
  "Go to `point-max'."
  (declare (ftype (function () integer)))
  (goto-char (point-max)))

;;;;; Definitions
(defun d-emacs-base-definition-name ()
...
Works for definition types in `d-emacs-base-definition-types-list'."
  ...
  (condition-case nil
      (let* ((defn (d-emacs-base-read-region))
             ...
             (name (nth 1 defn)))
        (if (member first-symbol d-emacs-base-definition-types-list)
            name))
...
;;;;; Invisibility
(defun d-emacs-base-save-invisible-overlays ()
...
(defun d-emacs-base-remove-invisible-overlays ()
  "Remove all overlays with `invisible' property from the entire buffer."
  (dolist (ov (overlays-in (point-min) (point-max)))
    (when (overlay-get ov 'invisible)
      (delete-overlay ov))))

(defun d-emacs-base-restore-invisible-overlays (saved-overlays)
...
(defun d-emacs-base-save-invisible-regions ()
...
(defun d-emacs-base-remove-invisible-text-properties ()
...
(defmacro d-emacs-base-with-all-visible (&rest body)
  ...
  `(let ((saved-overlays (d-emacs-base-save-invisible-overlays))
     ...
     (unwind-protect
         (progn
           (d-emacs-base-remove-invisible-overlays)
           (d-emacs-base-remove-invisible-text-properties)
           ,@body)
       (d-emacs-base-restore-invisible-overlays saved-overlays)
       (d-emacs-base-restore-invisible-text-properties saved-regions))))

;;;;; Commands
(defun d-emacs-base-fill-current-docstring ()
  "Fill the docstring of the current definition.

Works for definitions in `d-emacs-base-definition-types-list'. Works for the
              ...
              (progn (forward-char)
                     (let ((fillbeg (if (< (line-beginning-position) beg)
                       ...
                       (fill-region fillbeg (or end (progn (goto-char beg)
                                                           ...
                                                           (point))))))))
    (save-excursion
      (d-emacs-base-beginning-of-docstring)
      (d-emacs-base-fill-string-at-point-like-docstring))))

(defun d-emacs-base-fill-docstrings-in-buffer ()
  "Fill the docstrings in the current buffer.

Works for definitions in `d-emacs-base-definition-types-list'. If the first line
    ...
    (d-emacs-base-with-all-visible
     ...
     (while (re-search-forward
             (eval `(rx line-start
                        ...
                        (1+ (not (any space)))))
             nil t)
       (d-emacs-base-fill-current-docstring))
     (end-of-defun))))

(defun d-emacs-base-trim-lines ()
  "Remove whitespace at the end of each line in the current buffer."
  (declare (ftype (function ()
                            ;; void  ; Compiler complains.
                            t)))
  (interactive)
  (save-excursion (d-emacs-base-with-all-visible
                   (d-emacs-base-goto-min)
                   (while (re-search-forward (rx (+ blank) line-end) nil t)
                     (replace-match "")))))

(defun d-emacs-base-search-at-line-start (str &optional withcomments)
  "Search for an occurrence of STR at the start of a line.

Unlike normal `search-forward', `d-emacs-base-search-at-line-start' returns nil
