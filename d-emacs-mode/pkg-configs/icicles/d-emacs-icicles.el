;;;  d-emacs-icicles.el ---  Evaluated code for Daselt's Icicles Module      -*- lexical-binding: t; -*-
...
in this directory that contain `customs' in their name."
  (d-emacs-dirs-act-on-pkg-files-by-type-and-maybe-kill
   `(((lambda (filepath)
                     ...
                     (mapcar
                      (lambda (bind)
                        (let ((val (eval (cdr bind))))
                          (cons
                           (kbd (d-emacs-bind-string bind t t))
                           (if (atom val)
                               (list val t)
                             val))))
                      body))))
      ...
      . (lambda (filepath) (and (string-match-p "dbl" filepath)
                           (string-match-p "customs" (file-name-base filepath))))))
   (concat d-emacs-dirs-pkg-configs-directory "icicles/")))
...
(defun d-emacs-icicles--reset-key-customs-from-backups ()
   ...
   (lambda (filepath) (td-emacs-dirs-act-on-sexps-in-file
                  filepath
                  (lambda () (let* ((blist (d-emacs-read-region))
                               (icilist (d-emacs-bind-head blist))
                          ...
                          (if icilist
                            ...
                            (mapcar (lambda (bind)
                                    ...
                                    body))))))
   (concat d-emacs-dirs-pkg-configs-directory "icicles/")
   (lambda (filepath) (and (d-emacs-bind-bindlist-p filepath)
                      (string-match-p "customs" (file-name-base filepath))))))
...
(add-hook 'd-emacs-mode-hook
          (lambda ()
            (unless d-emacs-mode
              (d-emacs-icicles--reset-key-customs-from-backups))))

;;;; Provide
(provide 'd-emacs-icicles)
;;;  d-emacs-icicles.el ends here
