...
(defconst d-emacs-xkb-remaining-char-mappings
...
(let ((layrx (rx string-start
  ...
  (cl-flet* ((namecore (layout) (let ((layname (symbol-name layout)))
    ...
    (with-eval-after-load 'd-emacs-xkb-customs
      ...
      (defconst d-emacs-xkb-modifier-layer
      ...
      (defconst d-emacs-xkb-reduced-modifier-layer
      ...
      (defvaralias 'd-emacs-xkb-layer-0 (if d-dfk-dual-functions-outside-main-keys
                                            'd-emacs-xkb-reduced-modifier-layer
                                          'd-emacs-xkb-modifier-layer))

      (mapc (lambda (layout)
              (eval 
               `(defconst ,(extlaysymb layout)
                  (append (list d-emacs-xkb-layer-0) ,layout)
...
Generated from the first layer of the main layout.")
      ...
      (defconst d-emacs-xkb-extended-key-coords 
        (mapcar (lambda (indrow)
                  (mapcar (lambda (indplace)
                            (let ((fullcoords (d-emacs-coords-rel-to-abs
                                               (list 0 (car indrow) (car indplace))
                                               t)))
                              (list (nth 1 fullcoords)
                                    (nth 2 fullcoords))))
...
(provide 'd-emacs-xkb-constants)
;;; d-emacs-xkb-constants.el ends here
